...
class PluginsManager {
  ...
  public function quickSearchScript(){
    global $CLIENT_ROOT;
    $html = '<link href="'.$CLIENT_ROOT.'/css/jquery-ui.css" type="text/css" rel="Stylesheet" />'.PHP_EOL;
		$html .= "<script type='text/javascript'>if(!window.jQuery){".PHP_EOL;
    $html .= 'var jqresource = document.createElement("script");'.PHP_EOL;
    $html .= 'jqresource.src = "'.$CLIENT_ROOT.'/js/jquery.js";'.PHP_EOL;
    $html .= 'document.getElementsByTagName("head")[0].appendChild(jqresource);'.PHP_EOL;
    $html .= 'jqresource.onload = function(){'.PHP_EOL;
    $html .= 'var jquiresource = document.createElement("script");'.PHP_EOL;
    $html .= 'jquiresource.src = "'.$CLIENT_ROOT.'/js/jquery-ui.js";'.PHP_EOL;
    $html .= 'document.getElementsByTagName("head")[0].appendChild(jquiresource);'.PHP_EOL;
    $html .= 'jquiresource.onload = function() {initializeQuickSearch();};};}'.PHP_EOL;
    $html .= 'else{$(document).ready(function() {'.PHP_EOL;
    $html .= 'function split( val ) {'.PHP_EOL;
    $html .= 'return val.split( /,\s*/ );}'.PHP_EOL;
    $html .= 'function extractLast( term ) {'.PHP_EOL;
    $html .= 'return split( term ).pop();}'.PHP_EOL;
    $html .= '$( "#quicksearchtaxon" )'.PHP_EOL;
    $html .= '.bind( "keydown", function( event ) {'.PHP_EOL;
    $html .= 'if ( event.keyCode === $.ui.keyCode.TAB &&'.PHP_EOL;
    $html .= '$( this ).data( "autocomplete" ).menu.active ) {'.PHP_EOL;
    $html .= 'event.preventDefault();}})'.PHP_EOL;
    $html .= '.autocomplete({'.PHP_EOL;
    $html .= 'source: function( request, response ) {'.PHP_EOL;
    $html .= '$.getJSON( "'.$CLIENT_ROOT.'/collections/rpc/taxalist.php", {'.PHP_EOL;
    $html .= 'term: extractLast( request.term ), t: function() { return document.quicksearch.taxon.value; }}, response );},'.PHP_EOL;
    $html .= 'appendTo: "#quicksearchdiv",'.PHP_EOL;
    $html .= 'search: function() {'.PHP_EOL;
    $html .= 'var term = extractLast( this.value );'.PHP_EOL;
    $html .= 'if ( term.length < 4 ) {'.PHP_EOL;
    $html .= 'return false;}},'.PHP_EOL;
    $html .= 'focus: function() {'.PHP_EOL;
    $html .= 'return false;},'.PHP_EOL;
    $html .= 'select: function( event, ui ) {'.PHP_EOL;
    $html .= 'var terms = split( this.value );'.PHP_EOL;
    $html .= 'terms.pop();'.PHP_EOL;
    $html .= 'terms.push( ui.item.value );'.PHP_EOL;
    $html .= 'this.value = terms.join( ", " );'.PHP_EOL;
    $html .= 'return false;}},{});});}'.PHP_EOL;
    $html .= 'function initializeQuickSearch(){'.PHP_EOL;
    $html .= 'function split( val ) {'.PHP_EOL;
    $html .= 'return val.split( /,\s*/ );}'.PHP_EOL;
    $html .= 'function extractLast( term ) {'.PHP_EOL;
    $html .= 'return split( term ).pop();}'.PHP_EOL;
    ...
    $html .= 'this.value = terms.join( ", " );'.PHP_EOL;
    $html .= 'return false;}},{});}'.PHP_EOL;
        $html .= 'function verifyQuickSearch(f){'.PHP_EOL;
    $html .= 'if(document.getElementById("quicksearchtaxon").value == ""){'.PHP_EOL;
    $html .= 'alert("Please enter a scientific name to search for.");'.PHP_EOL;
    $html .= 'return false;}'.PHP_EOL;
    $html .= 'return true;}'.PHP_EOL;
    $html .= '</script>'.PHP_EOL;
    $html .= '<form name="quicksearch" id="quicksearch" action="'.$CLIENT_ROOT.'/taxa/index.php" method="get" onsubmit="return verifyQuickSearch(this.form);">'.PHP_EOL;
return $html;
  }

  public function quickSearchDiv($searchText,$class = ''){
    $html = '';
    if($searchText && $searchText != ''){
      $html .= '<div id="quicksearchtext" class="'.$class.'"><b>'.$searchText.'</b></div>';
    }
    return $html;
  }

  public function quickSearchInput($placeholderText,$class = ''){
    return '<div class="'.$class.'"><input type="text" name="taxon" placeholder="'.$placeholderText.'" id="quicksearchtaxon" title="Enter taxon name here." /></div>';
  }

  public function quickSearchButton($buttonText,$class = ''){
    return '<button name="formsubmit"  id="quicksearchbutton" type="submit" class="'.$class.'" value="Search Terms">'.$buttonText.'</button></form>';
  }

	public function createQuickSearch($buttonText,$searchText='',$placeholderText=''){
    $html = $this->quickSearchScript();
    $html .= $this->quickSearchDiv($searchText);
		$html .= $this->quickSearchInput($placeholderText);
    $html .= $this->quickSearchButton($buttonText);
		return $html;
	}
}
?>
